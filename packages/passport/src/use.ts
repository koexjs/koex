...
export function usePassport(app: App, options: IUsePassport) {
  ...
  const authenticatePath = format(authenticatePathPattern, {
    prefix: authPathPrefix,
  });
  ...
  const onUnauthorized =
    _options.onUnauthorized || createDefaultOnUnauthorized({ loginPath });
  const onAuthorized =
    _options.onAuthorized || createDefaultOnAuthorized({ loginPath });
  ...
  app.get(
    ...
    passport.callback({
      async onFail(error: any, ctx) {
        ...
        return ctx.redirect(
          `${loginPath}?error=${encodeURIComponent(
            code,
